...
public class ShiftViewActivity extends Fragment {
    ...
    private final List<Shift> Availableshifts = new ArrayList<>();
    private final List<Shift> selectedShifts  = new ArrayList<>();
    private final ArrayList<View> views           = new ArrayList<View>();

    private final int                  maxShiftsPerDay          = 4; // MAXIMUM shifts per day in the week (to know the number of
    // table rows)
    private final View.OnClickListener shiftButtonClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View view) {
            ...
            if (button.getTag() == null || !((boolean) button.getTag())) {
                button.getBackground().setColorFilter(ContextCompat.getColor(requireContext(),
                                                                             R.color.purple_200),
    ...
    private final View.OnClickListener sendButtonClickListener  = new View.OnClickListener() {
    ...
    List<Shift> createListOfShifts(List<Shift> availableshifts) {
        ...
        for (int i = 0; i < 7 * maxShiftsPerDay; i++) {
            ...
            Availableshifts.add(new Shift(dateString, 2, new ArrayList<>(), i + 2, startHour,
                                          duration));
        ...
        for (int i = 0; i <= maxShiftsPerDay; i++) {
            ...
            for (int j = 0; j < 7; j++) {
                if (row.getChildAt(j) instanceof Button) {
                    ...
                    if (button.getTag() != null && (boolean) button.getTag()) {
                        int shiftIndex = (i) * maxShiftsPerDay + j; // subtract 1 because the
